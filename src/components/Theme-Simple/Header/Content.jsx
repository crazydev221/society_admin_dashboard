...
const Content = ({ location, inboxHeader, setInboxHeader }) => {
  ...
  const customDropdown = path.includes("/email") || path.includes("/compose_email")
    ? 
    (<a href="#" className="toggle-secondary-sidebar" onClick = {() =>setInboxHeader(!inboxHeader)}>
      <span className="d-flex align-items-center">
          Inbox <span className="text-info">(12)</span> <span className="pg-icon">drop_down</span>
      </span>
    </a>) 
      : null;

    const emailEdit = path.includes("/email") || path.includes("/compose_email")
      ? 
      (<a href="#" id="mark-email" className="mark-email m-l-10 btn-icon-link"><i className="pg-icon">edit</i></a>) 
      : null;

  const [shouldShowProfile, setShouldShowProfile] = useState(false);
  const dispatch = useDispatch();
  const { toggleQuickViewOpen, toggleMobileSideBarOpen, toggleMobileSideBarClose } = QuickViewAction;
  ...
  return (
    <React.Fragment>
      ...
      <div className="">
        ...
        <a href="#" className="btn btn-link text-primary m-l-20 d-none d-lg-inline-flex d-xl-inline-flex">Add New Item</a>
      ...
      <div className="d-flex align-items-center">
        ...
        <div className="pull-left p-r-10 fs-14 d-lg-inline-block d-none m-l-20">
          <span className="semi-bold">David</span> <span className="text-color">Nest</span>
        </div>
        <div
